...
from QPSLClass.QPSLComboBox import QPSLComboBox
...
class QPSLWaveMaker(QPSLHorizontalGroupList):
    ...
    def __init__(self, parent: QWidget, object_name: str):
        ...
        self.setupUi()
        self.setupLogic()
        self.on_choose_wave_type(wave_type="square")
    ...
    def setupUi(self):
        ...
        self.cbox_wave_type.set_texts(texts=[
            "square","constant", "sin",  "triangle", "trapezoid", "bitrapezoid"
        ])
        self.box_para.add_widget(
        ...
        self.box_para.add_widget(widget=QPSLSpinBox(self.box_para,
                                                    ...
                                                    value=0,
                                                    prefix="phase:"))
        self.box_para.add_widget(widget=QPSLDoubleSpinBox(self.box_para,
                                                    object_name="spin_phase",
                                                    min=0,
                                                    max=1,
                                                    value=0.5,
    ...
    def on_choose_wave_type(self, wave_type: str):
        ...
        if wave_type == "constant":
            ...
            if wave_type == "sin":
                ...
                self.spin_para.show()
                if wave_type =="square":
                    self.spin_phase.setPrefix("dealy:")
                    self.spin_para.setPrefix("ratio:")
                    self.spin_para.setSingleStep(0.1)
                elif wave_type == "trapezoid" or wave_type == "bitrapezoid":
    ...
    def make_wave(self) -> Optional[np.ndarray]:
        ...
        if wave_type == "constant":
            ...
            return numpy_array_shift(array=np.array([min] * int (ratio * cycle) + [max] *
                                                    (cycle - int (ratio * cycle))),
                                     right_shift=phase + cycle - int(ratio * cycle))
        elif wave_type == "triangle":
            min, max, cycle, phase, ratio = self.spin_min.value(
            ), self.spin_max.value(), self.spin_cycle.value(
            ), self.spin_phase.value(), self.spin_para.value()
